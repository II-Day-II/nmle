...
impl ApplicationState {
    pub async fn new(window: Arc<Window>) -> Self {
        ...
        renderer.add_global_buffer("camera".into(), bytemuck::cast_slice(camera.get_matrix().as_col_slice()), BufferUsages::UNIFORM | BufferUsages::COPY_DST);
        renderer.add_pass("Default".into());
        debug!("Application state initialized");
        Self { renderer, camera, theta: 0.0 }
    }
    ...
    pub fn update(&mut self, dt_seconds: f64) {
        ...
        let view_proj = self.camera.get_matrix();
